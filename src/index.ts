...
// Create headers with non-null API key
...
async function performLocalSearch(query: string, count: number = 5) {
  ...
  const webResponse = await fetch(webUrl, { headers: createHeaders() });
...
app.listen(PORT, () => {
  console.log(`Brave Search MCP Server listening on port ${PORT}`);
